void exportbmp (char *dateiname)
{
  ...
  for(i=0;i<=maxzaehler;i++)
  {
    if(bruch && bruchou)
    {
      ...
      cairo_move_to(cr, alloz->x-FensterRandLinks, alloz->y-FensterRandOben-paddingbmp);
      PangoLayout *layout = pango_cairo_create_layout (cr);
      pango_layout_set_text (layout, gtk_label_get_text(GTK_LABEL(zaehlerlabel[i])), -1);
      PangoFontDescription *desc = pango_font_description_from_string (schriftart);
      pango_layout_set_font_description (layout, desc);
      pango_cairo_show_layout (cr, layout);
      cairo_move_to(cr, allon->x-FensterRandLinks, allon->y-FensterRandOben-paddingbmp);
    ...
    else
    {
      ...
      if(winkel[i] >= 0)
      {
        cairo_move_to(cr,allo->x-FensterRandLinks+(allo->width*tan(winkel[i]*G_PI/180.0)-allo->height)/(tan(winkel[i]*G_PI/180.0)-1.0/tan(winkel[i]*G_PI/180.0))-(paddingbmp+hoehevordrehung[i])*sin(winkel[i]*G_PI/180), allo->y+allo->height-FensterRandOben-(paddingbmp+hoehevordrehung[i])*cos(winkel[i]*G_PI/180));
      }
      else
      {
        cairo_move_to(cr,allo->x-FensterRandLinks-(paddingbmp+hoehevordrehung[i])*sin(winkel[i]*G_PI/180), allo->y+(allo->width-allo->height/tan(-winkel[i]*G_PI/180.0))/(tan(-winkel[i]*G_PI/180.0)-1.0/tan(-winkel[i]*G_PI/180.0))-FensterRandOben-(paddingbmp+hoehevordrehung[i])*cos(winkel[i]*G_PI/180));
      }
  ...
  gdk_pixbuf_save (bild, dateiname, "bmp", NULL, NULL);
